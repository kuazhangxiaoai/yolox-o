...
from utils.common import cvtColor, preprocess_input, letterbox, mixup,random_perspective,poly_filter
from utils.utils_bbox import xyxy2cxcywhab, xywhab2xyxy
...
def drawOneImg(img, label, save_path=False, color=(0,0,255), withClass=True):
    pts = label[:, : -1] if withClass else label
...
class DotaDataset(Dataset):
    ...
    def postprocessing(self, img, targets, hsv_prob=1.0, filp_prob=0.5, swap=(2, 0, 1)):
        ...
        image_t = np.transpose(preprocess_input(np.array(image_t, dtype=np.float32)), swap)
